...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::PlanComparisonJoin(LogicalComparisonJoin &op) {
	...
	auto make_policy = [&]() -> shared_ptr<DefaultOperatorMemoryPolicy> {
		...
		return make_shared_ptr<DefaultOperatorMemoryPolicy>(
		    operator_budget, opts.operator_sort_input_buffer_ratio, opts.operator_nlj_input_buffer_ratio,
		    opts.operator_nlj_short_table_buffer_ratio, opts.operator_kway_merge_k, opts.operator_kway_merge_adaptive,
		    opts.operator_kway_merge_input_buffer_ratio);
	};
	...
	// Force nested loop join if the flag is set (for testing/debugging)
	if (client_config.force_nested_loop_join && !op.conditions.empty()) {
		if (PhysicalNestedLoopJoin::IsSupported(op.conditions, op.join_type)) {
			if (db_config.options.enable_operator_memory_optimizations) {
				...
				plan = make_uniq<PhysicalNestedLoopJoinOptimized>(op, std::move(left), std::move(right),
				                                                  std::move(op.conditions), op.join_type,
				                                                  op.estimated_cardinality, std::move(policy));
			} else {
				plan = make_uniq<PhysicalNestedLoopJoin>(op, std::move(left), std::move(right),
				                                         std::move(op.conditions), op.join_type,
				                                         op.estimated_cardinality);
			}
		} else {
			// Fallback to blockwise NL for unsupported cases (e.g., complex expressions)
			...
			if (db_config.options.enable_operator_memory_optimizations) {
				auto policy = make_policy();
				plan = make_uniq<PhysicalBlockwiseNLJoinOptimized>(op, std::move(left), std::move(right),
				                                                   std::move(condition), op.join_type,
				                                                   op.estimated_cardinality, std::move(policy));
			} else {
