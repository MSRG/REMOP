...
static inline void BumpTransferReadRoundVanilla(ClientContext &client) {
...
static inline void BumpTransferWriteRoundVanilla(ClientContext &client) {
	auto &buffer_pool = BufferManager::GetBufferManager(client).GetBufferPool();
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
	...
	do {
		if (state.fetch_next_right) {
			...
			if (gstate.right_condition_data.Scan(state.condition_scan_state, state.right_condition)) {
				...
				// One logical RHS chunk scan in vanilla NLJ.
				BumpTransferReadRoundVanilla(context.client);
			} else {
		...
		if (state.fetch_next_left) {
			...
			state.left_condition.Reset();
			state.lhs_executor.Execute(input, state.left_condition);
			// One logical LHS chunk execution in vanilla NLJ.
			BumpTransferReadRoundVanilla(context.client);
			...
			// Initial RHS chunk scan for this LHS chunk.
			BumpTransferReadRoundVanilla(context.client);
		...
		// we have finished resolving the join conditions
			if (match_count > 0) {
				...
				// One logical output return in vanilla NLJ.
				BumpTransferWriteRoundVanilla(context.client);
