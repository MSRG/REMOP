...
BufferPool::BufferPool(idx_t maximum_memory, bool track_eviction_timestamps)
    : current_memory(0), maximum_memory(maximum_memory), track_eviction_timestamps(track_eviction_timestamps),
      ...
      evict_queue_insertions(0), total_dead_nodes(0), profiling_enabled(false) {
...
void BufferPool::UpdateUsedMemory(MemoryTag tag, int64_t size) {
	...
	// Update peak memory usage if profiling is enabled
	if (profiling_enabled && profiling_metrics) {
		idx_t new_memory = current_memory.load();
		idx_t peak = profiling_metrics->peak_memory_usage.load();
		while (new_memory > peak &&
		       !profiling_metrics->peak_memory_usage.compare_exchange_weak(peak, new_memory)) {
			// Loop until we successfully update or peak is higher
		}
	}
}
...
BufferPool::EvictionResult BufferPool::EvictBlocks(MemoryTag tag, idx_t extra_memory, idx_t memory_limit,
	...
	// Track eviction attempt
	if (profiling_enabled && profiling_metrics) {
		profiling_metrics->eviction_attempt_count++;
	}
	...
	if (current_memory <= memory_limit) {
		if (profiling_enabled && profiling_metrics) {
			profiling_metrics->eviction_success_count++;
		}
	...
	IterateUnloadableBlocks([&](BufferEvictionNode &, const shared_ptr<BlockHandle> &handle) {
		...
		if (buffer && handle->buffer->AllocSize() == extra_memory) {
			...
			*buffer = handle->UnloadAndTakeBlock();
			evicted_count++;
			if (profiling_enabled && profiling_metrics) {
				profiling_metrics->buffer_reuse_count++;
			}
			found = true;
			return false;
		}
	...
	if (!found) {
		r.Resize(0);
		if (profiling_enabled && profiling_metrics) {
			profiling_metrics->eviction_failure_count++;
		}
	} else {
		if (profiling_enabled && profiling_metrics) {
			profiling_metrics->eviction_success_count++;
			profiling_metrics->blocks_evicted_count += evicted_count;
		}
	}
...
void BufferPool::PurgeIteration(const idx_t purge_size) {
	...
	if (profiling_enabled && profiling_metrics) {
		profiling_metrics->purge_nodes_processed += actually_dequeued;
	}
...
void BufferPool::SetProfiling(bool enable) {
	if (enable && !profiling_metrics) {
		profiling_metrics = make_uniq<BufferPoolMetrics>();
#ifdef REMON_MALLOC
		// Set global allocator metrics for timing
		SetGlobalAllocatorMetrics(profiling_metrics.get());
#endif
	} else if (!enable) {
#ifdef REMON_MALLOC
		SetGlobalAllocatorMetrics(nullptr);
#endif
		profiling_metrics.reset();
	}
	profiling_enabled = enable;
}

bool BufferPool::IsProfilingEnabled() const {
	return profiling_enabled;
}

const BufferPoolMetrics *BufferPool::GetMetrics() const {
	return profiling_metrics.get();
}
...
string BufferPool::GetProfilingReport() const {
	if (!profiling_metrics) {
		return "Profiling is not enabled";
	}
	return profiling_metrics->ToString();
}
