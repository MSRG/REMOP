...
#include "duckdb/common/remon_allocator.hpp"
#include <chrono>
...
#ifdef REMON_MALLOC
	// If block was demoted to REMON (buffer exists but state is UNLOADED),
	// we need to clean up remote/disk resources before the buffer is freed
	if (buffer && state == BlockState::BLOCK_UNLOADED) {
		RemonCleanupEvicted(buffer->InternalBuffer());
	}
#endif
...
BufferHandle BlockHandle::Load(shared_ptr<BlockHandle> &handle, unique_ptr<FileBuffer> reusable_buffer) {
...
#ifdef REMON_MALLOC
	// Check if this block was demoted to REMON (buffer exists but state is UNLOADED)
	if (handle->buffer) {
		// Block was demoted to REMON - activate it (fetch from remote/disk if evicted)
		auto swap_start = std::chrono::steady_clock::now();
		RemonActivateSync(handle->buffer->InternalBuffer());
		auto swap_end = std::chrono::steady_clock::now();
		auto elapsed_us = std::chrono::duration_cast<std::chrono::microseconds>(swap_end - swap_start).count();

		// Record swap read metrics (treat REMON activation as swap read)
		auto &buffer_pool = handle->block_manager.buffer_manager.GetBufferPool();
		if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
			auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
			metrics->swap_read_time_us += static_cast<uint64_t>(elapsed_us);
			metrics->swap_read_count++;
			metrics->swap_read_bytes += handle->memory_usage;
			metrics->swap_read_per_tag[uint8_t(handle->tag)]++;
		}
	...
	if (handle->block_id < MAXIMUM_BLOCK) {
		...
		if (handle->can_destroy) {
			...
			// Record swap read time and count in profiler
			auto &buffer_pool = block_manager.buffer_manager.GetBufferPool();
			if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
				auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
				metrics->swap_read_time_us += static_cast<uint64_t>(elapsed_us);
				metrics->swap_read_count++;
...
#ifdef REMON_MALLOC
		// Demote block to REMON (hint for eviction to remote memory)
		// Time the demote operation
		auto swap_start = std::chrono::steady_clock::now();
		RemonDemote(buffer->InternalBuffer());
		auto swap_end = std::chrono::steady_clock::now();
		auto elapsed_us = std::chrono::duration_cast<std::chrono::microseconds>(swap_end - swap_start).count();

		// Record swap write metrics (treat REMON demote as swap write)
		auto &buffer_pool = block_manager.buffer_manager.GetBufferPool();
		if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
			auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
			metrics->swap_write_time_us += static_cast<uint64_t>(elapsed_us);
			metrics->swap_write_count++;
			metrics->swap_write_bytes += buffer->AllocSize();
			metrics->swap_write_per_tag[uint8_t(tag)]++;
		}

		// Keep buffer in BlockHandle (don't return for reuse)
		memory_charge.Resize(0);
		state = BlockState::BLOCK_UNLOADED;
		return nullptr;
#else
		// temporary block that cannot be destroyed: write to temporary file
		// Time the swap write operation
		auto swap_start = std::chrono::steady_clock::now();
		block_manager.buffer_manager.WriteTemporaryBuffer(tag, block_id, *buffer);
		auto swap_end = std::chrono::steady_clock::now();
		auto elapsed_us = std::chrono::duration_cast<std::chrono::microseconds>(swap_end - swap_start).count();

		// Record swap write time and count in profiler
		auto &buffer_pool = block_manager.buffer_manager.GetBufferPool();
		if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
			auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
			metrics->swap_write_time_us += static_cast<uint64_t>(elapsed_us);
...
#ifdef REMON_MALLOC
	// With REMON, we can always unload to remote memory - no temp directory needed
	return true;
#else
