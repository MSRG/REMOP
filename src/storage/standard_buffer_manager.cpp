...
BufferHandle StandardBufferManager::Allocate(MemoryTag tag, idx_t block_size, bool can_destroy,
	...
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		...
		metrics->allocation_bytes += block_size;
	}
...
void StandardBufferManager::WriteTemporaryBuffer(MemoryTag tag, block_id_t block_id, FileBuffer &buffer) {
	...
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		...
		metrics->swap_write_bytes += buffer.size;
...
unique_ptr<FileBuffer> StandardBufferManager::ReadTemporaryBuffer(MemoryTag tag, block_id_t id,
	...
	if (temporary_directory.handle->GetTempFile().HasTemporaryBuffer(id)) {
		...
		evicted_data_per_tag[uint8_t(tag)] -= Storage::BLOCK_SIZE;
		result = temporary_directory.handle->GetTempFile().ReadTemporaryBuffer(id, std::move(reusable_buffer));
	} else {
		idx_t block_size;
		...
		result = ReadTemporaryBufferInternal(*this, *handle, sizeof(idx_t), block_size, std::move(reusable_buffer));
	...
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		...
		metrics->swap_read_bytes += bytes_read;
		metrics->swap_read_per_tag[uint8_t(tag)]++;
	}
...
void StandardBufferManager::SetProfiling(bool enable) {
	buffer_pool.SetProfiling(enable);
}

bool StandardBufferManager::IsProfilingEnabled() const {
	return buffer_pool.IsProfilingEnabled();
}

const BufferPoolMetrics *StandardBufferManager::GetProfilingMetrics() const {
	return buffer_pool.GetMetrics();
}

void StandardBufferManager::ResetProfilingMetrics() {
	buffer_pool.ResetMetrics();
}

string StandardBufferManager::GetProfilingReport() const {
	return buffer_pool.GetProfilingReport();
}
