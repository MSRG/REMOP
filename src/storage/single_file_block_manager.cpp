...
void SingleFileBlockManager::Read(Block &block) {
	...
	auto &buffer_pool = BufferManager::GetBufferManager(db).GetBufferPool();
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
		metrics->db_read_count++;
		metrics->db_read_bytes += block.size;
		metrics->db_read_time_us += static_cast<uint64_t>(elapsed_us);
	}
}
...
void SingleFileBlockManager::Write(FileBuffer &buffer, block_id_t block_id) {
	...
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
		metrics->db_write_count++;
		metrics->db_write_bytes += buffer.size;
		metrics->db_write_time_us += static_cast<uint64_t>(elapsed_us);
	}
}
