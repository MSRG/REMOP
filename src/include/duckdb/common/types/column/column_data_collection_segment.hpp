...
public:
	...
	//! Pin all BM blocks needed for chunks in range [start_chunk, end_chunk)
	//! Returns handles map - blocks stay pinned as long as handles are held
	unordered_map<uint32_t, BufferHandle> PinChunkRange(idx_t start_chunk, idx_t end_chunk);
