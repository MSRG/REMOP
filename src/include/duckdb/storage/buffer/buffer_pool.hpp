...
public:
	...
	//! Get profiling metrics (returns nullptr if profiling is disabled)
	const BufferPoolMetrics *GetMetrics() const;
	//! Reset profiling metrics
	void ResetMetrics();
	//! Get formatted profiling report
	string GetProfilingReport() const;
...
protected:
	...
	//! Profiling metrics (nullptr if profiling is disabled)
	unique_ptr<BufferPoolMetrics> profiling_metrics;
