...
struct DBConfigOptions {
	...
	//! Enable optimized operator wrappers controlled by OperatorMemoryPolicy
	bool enable_operator_memory_optimizations = false;
	//! Memory budget for optimized operators (0 = use memory_limit)
	idx_t operator_memory_budget = 0;
	//! Sort merge input buffer ratio (output = 1 - input)
	double operator_sort_input_buffer_ratio = 0.6;
	//! NLJ input buffer ratio (output = 1 - input)
	double operator_nlj_input_buffer_ratio = 0.4;
	//! NLJ inner/short table buffer ratio (outer/long = 1 - inner)
	double operator_nlj_short_table_buffer_ratio = 0.6;
	//! K-way merge fan-in (minimum 2)
	idx_t operator_kway_merge_k = 16;
	//! Whether to auto-adjust K for k-way merge based on memory budget
	bool operator_kway_merge_adaptive = true;
	//! K-way merge input buffer ratio (output = 1 - input)
	double operator_kway_merge_input_buffer_ratio = 0.67;
