...
//===--------------------------------------------------------------------===//
void ForceNestedLoopJoin::ResetLocal(ClientContext &context) {
	ClientConfig::GetConfig(context).force_nested_loop_join = ClientConfig().force_nested_loop_join;
}

void ForceNestedLoopJoin::SetLocal(ClientContext &context, const Value &input) {
	ClientConfig::GetConfig(context).force_nested_loop_join = input.GetValue<bool>();
}

Value ForceNestedLoopJoin::GetSetting(const ClientContext &context) {
	return Value::BOOLEAN(ClientConfig::GetConfig(context).force_nested_loop_join);
}
...
//===--------------------------------------------------------------------===//
// Operator Memory Optimizations
//===--------------------------------------------------------------------===//
void OperatorMemoryOptimizations::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	config.options.enable_operator_memory_optimizations = DBConfigOptions().enable_operator_memory_optimizations;
}

void OperatorMemoryOptimizations::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	bool enable = input.GetValue<bool>();
	if (enable && !config.options.enable_operator_memory_optimizations) {
		// Enabling: validate ratios and reset to defaults if invalid
		auto &opts = config.options;
		auto defaults = DBConfigOptions();
		if (opts.operator_sort_input_buffer_ratio < 0.0 || opts.operator_sort_input_buffer_ratio > 1.0) {
			Printer::Print(StringUtil::Format(
			    "Warning: operator_sort_input_buffer_ratio out of range [0.0, 1.0], using default %.2f",
			    defaults.operator_sort_input_buffer_ratio));
			opts.operator_sort_input_buffer_ratio = defaults.operator_sort_input_buffer_ratio;
		}
		if (opts.operator_nlj_input_buffer_ratio < 0.0 || opts.operator_nlj_input_buffer_ratio > 1.0) {
			Printer::Print(StringUtil::Format(
			    "Warning: operator_nlj_input_buffer_ratio out of range [0.0, 1.0], using default %.2f",
			    defaults.operator_nlj_input_buffer_ratio));
			opts.operator_nlj_input_buffer_ratio = defaults.operator_nlj_input_buffer_ratio;
		}
		if (opts.operator_nlj_short_table_buffer_ratio < 0.0 || opts.operator_nlj_short_table_buffer_ratio > 1.0) {
			Printer::Print(StringUtil::Format(
			    "Warning: operator_nlj_short_table_buffer_ratio out of range [0.0, 1.0], using default %.2f",
			    defaults.operator_nlj_short_table_buffer_ratio));
			opts.operator_nlj_short_table_buffer_ratio = defaults.operator_nlj_short_table_buffer_ratio;
		}
		if (opts.operator_kway_merge_k < 2) {
			Printer::Print(StringUtil::Format("Warning: operator_kway_merge_k must be >= 2, using default %llu",
			                                  static_cast<unsigned long long>(defaults.operator_kway_merge_k)));
			opts.operator_kway_merge_k = defaults.operator_kway_merge_k;
		}
		if (opts.operator_kway_merge_input_buffer_ratio < 0.0 || opts.operator_kway_merge_input_buffer_ratio > 1.0) {
			Printer::Print(StringUtil::Format(
			    "Warning: operator_kway_merge_input_buffer_ratio out of range [0.0, 1.0], using default %.2f",
			    defaults.operator_kway_merge_input_buffer_ratio));
			opts.operator_kway_merge_input_buffer_ratio = defaults.operator_kway_merge_input_buffer_ratio;
		}
	}
	config.options.enable_operator_memory_optimizations = enable;
}

Value OperatorMemoryOptimizations::GetSetting(const ClientContext &context) {
	return Value::BOOLEAN(DBConfig::GetConfig(context).options.enable_operator_memory_optimizations);
}

//===--------------------------------------------------------------------===//
// Operator Memory Budget
//===--------------------------------------------------------------------===//
void OperatorMemoryBudget::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	config.options.operator_memory_budget = DBConfigOptions().operator_memory_budget;
}

void OperatorMemoryBudget::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	config.options.operator_memory_budget = DBConfig::ParseMemoryLimit(input.ToString());
}

Value OperatorMemoryBudget::GetSetting(const ClientContext &context) {
	auto budget = DBConfig::GetConfig(context).options.operator_memory_budget;
	if (budget == 0) {
		return Value("0 (uses memory_limit)");
	}
	return Value(StringUtil::BytesToHumanReadableString(budget));
}

//===--------------------------------------------------------------------===//
// Operator Sort Input Buffer Ratio
//===--------------------------------------------------------------------===//
void OperatorSortInputBufferRatio::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_sort_input_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_sort_input_buffer_ratio = DBConfigOptions().operator_sort_input_buffer_ratio;
}

void OperatorSortInputBufferRatio::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_sort_input_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_sort_input_buffer_ratio = input.GetValue<double>();
}

Value OperatorSortInputBufferRatio::GetSetting(const ClientContext &context) {
	return Value::DOUBLE(DBConfig::GetConfig(context).options.operator_sort_input_buffer_ratio);
}

//===--------------------------------------------------------------------===//
// Operator NLJ Input Buffer Ratio
//===--------------------------------------------------------------------===//
void OperatorNLJInputBufferRatio::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_nlj_input_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_nlj_input_buffer_ratio = DBConfigOptions().operator_nlj_input_buffer_ratio;
}

void OperatorNLJInputBufferRatio::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_nlj_input_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_nlj_input_buffer_ratio = input.GetValue<double>();
}

Value OperatorNLJInputBufferRatio::GetSetting(const ClientContext &context) {
	return Value::DOUBLE(DBConfig::GetConfig(context).options.operator_nlj_input_buffer_ratio);
}

//===--------------------------------------------------------------------===//
// Operator NLJ Short Table Buffer Ratio
//===--------------------------------------------------------------------===//
void OperatorNLJShortTableBufferRatio::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_nlj_short_table_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_nlj_short_table_buffer_ratio = DBConfigOptions().operator_nlj_short_table_buffer_ratio;
}

void OperatorNLJShortTableBufferRatio::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_nlj_short_table_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_nlj_short_table_buffer_ratio = input.GetValue<double>();
}

Value OperatorNLJShortTableBufferRatio::GetSetting(const ClientContext &context) {
	return Value::DOUBLE(DBConfig::GetConfig(context).options.operator_nlj_short_table_buffer_ratio);
}

//===--------------------------------------------------------------------===//
// Operator NLJ Outer Table Buffer Ratio
//===--------------------------------------------------------------------===//
void OperatorNLJOuterTableBufferRatio::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_nlj_outer_table_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	// Keep inner/short as the canonical stored value.
	config.options.operator_nlj_short_table_buffer_ratio = DBConfigOptions().operator_nlj_short_table_buffer_ratio;
}

void OperatorNLJOuterTableBufferRatio::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_nlj_outer_table_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	auto outer_ratio = input.GetValue<double>();
	config.options.operator_nlj_short_table_buffer_ratio = 1.0 - outer_ratio;
}

Value OperatorNLJOuterTableBufferRatio::GetSetting(const ClientContext &context) {
	auto short_ratio = DBConfig::GetConfig(context).options.operator_nlj_short_table_buffer_ratio;
	return Value::DOUBLE(1.0 - short_ratio);
}

//===--------------------------------------------------------------------===//
// Operator K-Way Merge K
//===--------------------------------------------------------------------===//
void OperatorKWayMergeK::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_kway_merge_k while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_kway_merge_k = DBConfigOptions().operator_kway_merge_k;
}

void OperatorKWayMergeK::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_kway_merge_k while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	auto k_value = input.GetValue<int64_t>();
	config.options.operator_kway_merge_k = k_value > 0 ? NumericCast<idx_t>(k_value) : 0;
}

Value OperatorKWayMergeK::GetSetting(const ClientContext &context) {
	return Value::BIGINT(NumericCast<int64_t>(DBConfig::GetConfig(context).options.operator_kway_merge_k));
}

//===--------------------------------------------------------------------===//
// Operator K-Way Merge Adaptive
//===--------------------------------------------------------------------===//
void OperatorKWayMergeAdaptive::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_kway_merge_adaptive while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_kway_merge_adaptive = DBConfigOptions().operator_kway_merge_adaptive;
}

void OperatorKWayMergeAdaptive::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_kway_merge_adaptive while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_kway_merge_adaptive = input.GetValue<bool>();
}

Value OperatorKWayMergeAdaptive::GetSetting(const ClientContext &context) {
	return Value::BOOLEAN(DBConfig::GetConfig(context).options.operator_kway_merge_adaptive);
}

//===--------------------------------------------------------------------===//
// Operator K-Way Merge Input Buffer Ratio
//===--------------------------------------------------------------------===//
void OperatorKWayMergeInputBufferRatio::ResetGlobal(DatabaseInstance *db, DBConfig &config) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot reset operator_kway_merge_input_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_kway_merge_input_buffer_ratio = DBConfigOptions().operator_kway_merge_input_buffer_ratio;
}

void OperatorKWayMergeInputBufferRatio::SetGlobal(DatabaseInstance *db, DBConfig &config, const Value &input) {
	if (config.options.enable_operator_memory_optimizations) {
		throw InvalidInputException("Cannot change operator_kway_merge_input_buffer_ratio while "
		                            "operator_memory_optimizations is enabled. Disable it first.");
	}
	config.options.operator_kway_merge_input_buffer_ratio = input.GetValue<double>();
}

Value OperatorKWayMergeInputBufferRatio::GetSetting(const ClientContext &context) {
	return Value::DOUBLE(DBConfig::GetConfig(context).options.operator_kway_merge_input_buffer_ratio);
}
