...
unordered_map<uint32_t, BufferHandle> ColumnDataCollectionSegment::PinChunkRange(idx_t start_chunk, idx_t end_chunk) {
	unordered_map<uint32_t, BufferHandle> handles;

	// Only pin for buffer-managed allocators
	auto alloc_type = allocator->GetType();
	if (alloc_type != ColumnDataAllocatorType::BUFFER_MANAGER_ALLOCATOR &&
	    alloc_type != ColumnDataAllocatorType::HYBRID) {
		return handles;  // Empty - nothing to pin for in-memory allocator
	}

	// Clamp end_chunk to valid range
	idx_t actual_end = MinValue<idx_t>(end_chunk, chunk_data.size());

	// Collect all unique block IDs from chunks in range
	for (idx_t chunk_idx = start_chunk; chunk_idx < actual_end; chunk_idx++) {
		for (auto block_id : chunk_data[chunk_idx].block_ids) {
			if (handles.find(block_id) == handles.end()) {
				// Pin the block - uses friend access to ColumnDataAllocator::Pin()
				handles[block_id] = allocator->Pin(block_id);
			}
		}
	}
