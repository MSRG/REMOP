...
static inline void BumpTransferReadRound(BufferManager &buffer_manager, idx_t delta = 1) {
	...
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		...
		for (idx_t i = 0; i < delta; i++) {
			metrics->transfer_read_rounds++;
		}
	}
}

static inline void BumpTransferWriteRound(BufferManager &buffer_manager, idx_t delta = 1) {
	auto &buffer_pool = buffer_manager.GetBufferPool();
	if (buffer_pool.IsProfilingEnabled() && buffer_pool.GetMetrics()) {
		auto *metrics = const_cast<BufferPoolMetrics *>(buffer_pool.GetMetrics());
		for (idx_t i = 0; i < delta; i++) {
...
void MergeSorter::MergePartition() {
	...
	#endif
	// One logical output block is produced per 2-way merge partition.
	BumpTransferWriteRound(buffer_manager);
}
...
void MergeSorter::GetNextPartition() {
	...
	// One logical read round per input side for this 2-way merge partition.
	BumpTransferReadRound(buffer_manager, 2);
	state.l_start = l_end;
	state.r_start = r_end;
