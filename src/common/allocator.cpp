...
// Always include remon_allocator.hpp for timing helpers (works for both OS and REMON)
...
data_ptr_t Allocator::DefaultAllocate(PrivateAllocatorData *private_data, idx_t size) {
	auto *metrics = GetGlobalAllocatorMetrics();
	auto start = metrics ? std::chrono::steady_clock::now() : std::chrono::steady_clock::time_point();

	data_ptr_t result;
#ifdef REMON_MALLOC
	auto ptr = SafeRemonMalloc(size);
	if (!ptr) {
		throw std::bad_alloc();
	}
	result = data_ptr_cast(ptr);
#elif defined(USE_JEMALLOC)
	result = JemallocExtension::Allocate(private_data, size);
#else
	auto default_allocate_result = malloc(size);
	if (!default_allocate_result) {
		throw std::bad_alloc();
	}
	result = data_ptr_cast(default_allocate_result);
#endif

	if (metrics) {
		auto end = std::chrono::steady_clock::now();
		auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
		metrics->allocation_time_us += elapsed;
	}
	return result;
}
...
data_ptr_t Allocator::DefaultReallocate(PrivateAllocatorData *private_data, data_ptr_t pointer, idx_t old_size,
	...
	data_ptr_t result;
#ifdef REMON_MALLOC
	auto *new_addr = SafeRemonMalloc(size);
	if (!new_addr) {
		throw std::bad_alloc();
	}
	memcpy(new_addr, pointer, old_size < size ? old_size : size);
	SafeRemonFree(pointer);
	result = data_ptr_cast(new_addr);
#elif defined(USE_JEMALLOC)
	result = JemallocExtension::Reallocate(private_data, pointer, old_size, size);
#else
	result = data_ptr_cast(realloc(pointer, size));
#endif

	if (metrics) {
		auto end = std::chrono::steady_clock::now();
		auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();
		// Reallocate is effectively free + alloc, attribute to allocation
		metrics->allocation_time_us += elapsed;
	}
	return result;
}
...
#ifdef REMON_MALLOC
	// no-op: REMON manages its own memory
#elif defined(USE_JEMALLOC)
