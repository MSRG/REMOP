...
#if !defined(_WIN32) && defined(REMON_MALLOC)
static constexpr idx_t REMON_BOUNCE_BUFFER_CHUNK_SIZE = 1024ULL * 1024ULL;
#endif
...
void LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	while (nr_bytes > 0) {
		...
		if (bytes_read == -1 && errno == EFAULT) {
			auto chunk_size = MinValue<idx_t>(UnsafeNumericCast<idx_t>(nr_bytes), REMON_BOUNCE_BUFFER_CHUNK_SIZE);
			auto *bounce_buffer = reinterpret_cast<char *>(std::malloc(chunk_size));
			if (!bounce_buffer) {
				throw IOException("Could not read from file \"%s\": failed to allocate fallback read buffer",
				                  handle.path);
			}
			while (nr_bytes > 0) {
				auto to_read = MinValue<idx_t>(UnsafeNumericCast<idx_t>(nr_bytes), chunk_size);
				bytes_read = pread(fd, bounce_buffer, UnsafeNumericCast<size_t>(to_read),
				                   UnsafeNumericCast<off_t>(location));
				if (bytes_read == -1) {
					auto read_errno = errno;
					std::free(bounce_buffer);
					throw IOException("Could not read from file \"%s\": %s", {{"errno", std::to_string(read_errno)}},
					                  handle.path, strerror(read_errno));
				}
				if (bytes_read == 0) {
					std::free(bounce_buffer);
					throw IOException(
				...
				memcpy(read_buffer, bounce_buffer, UnsafeNumericCast<size_t>(bytes_read));
...
int64_t LocalFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	if (bytes_read == -1 && errno == EFAULT) {
		...
		auto *bounce_buffer = reinterpret_cast<char *>(std::malloc(chunk_size));
		if (!bounce_buffer) {
			throw IOException("Could not read from file \"%s\": failed to allocate fallback read buffer", handle.path);
		}
		auto read_buffer = char_ptr_cast(buffer);
		int64_t total_read = 0;
		while (total_read < nr_bytes) {
			auto to_read = MinValue<idx_t>(UnsafeNumericCast<idx_t>(nr_bytes - total_read), chunk_size);
			auto current_read = read(fd, bounce_buffer, UnsafeNumericCast<size_t>(to_read));
			if (current_read <= 0) {
				if (total_read == 0) {
			...
			memcpy(read_buffer + total_read, bounce_buffer, UnsafeNumericCast<size_t>(current_read));
			total_read += current_read;
			if (current_read < UnsafeNumericCast<int64_t>(to_read)) {
				bytes_read = total_read;
				break;
			}
			bytes_read = total_read;
...
void LocalFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	while (nr_bytes > 0) {
		...
		if (bytes_written == -1 && errno == EFAULT) {
			...
			auto *bounce_buffer = reinterpret_cast<char *>(std::malloc(chunk_size));
			if (!bounce_buffer) {
				throw IOException("Could not write file \"%s\": failed to allocate fallback write buffer", handle.path);
			}
			while (nr_bytes > 0) {
				auto to_write = MinValue<idx_t>(UnsafeNumericCast<idx_t>(nr_bytes), chunk_size);
				memcpy(bounce_buffer, write_buffer, UnsafeNumericCast<size_t>(to_write));
				bytes_written = pwrite(fd, bounce_buffer, UnsafeNumericCast<size_t>(to_write),
				                       UnsafeNumericCast<off_t>(location));
				if (bytes_written < 0) {
					auto write_errno = errno;
					std::free(bounce_buffer);
					throw IOException("Could not write file \"%s\": %s", {{"errno", std::to_string(write_errno)}},
					                  handle.path, strerror(write_errno));
				}
				if (bytes_written == 0) {
					auto write_errno = errno;
					std::free(bounce_buffer);
					throw IOException("Could not write to file \"%s\" - attempted to write 0 bytes: %s",
					                  {{"errno", std::to_string(write_errno)}}, handle.path, strerror(write_errno));
				}
...
#ifdef REMON_MALLOC
		if (current_bytes_written == -1 && errno == EFAULT) {
			auto chunk_size = MinValue<idx_t>(bytes_to_write, REMON_BOUNCE_BUFFER_CHUNK_SIZE);
			auto *bounce_buffer = reinterpret_cast<char *>(std::malloc(chunk_size));
			if (!bounce_buffer) {
				throw IOException("Could not write file \"%s\": failed to allocate fallback write buffer", handle.path);
			}
			...
			auto write_buffer = char_ptr_cast(buffer);
			while (total_written < UnsafeNumericCast<int64_t>(bytes_to_write)) {
				auto to_write =
				    MinValue<idx_t>(UnsafeNumericCast<idx_t>(bytes_to_write - total_written), chunk_size);
				memcpy(bounce_buffer, write_buffer + total_written, UnsafeNumericCast<size_t>(to_write));
				auto step_written = write(fd, bounce_buffer, to_write);
				if (step_written <= 0) {
					current_bytes_written = total_written > 0 ? total_written : step_written;
					break;
				}
				total_written += step_written;
				if (step_written < UnsafeNumericCast<int64_t>(to_write)) {
					break;
				}
			}
			if (total_written > 0) {
				current_bytes_written = total_written;
			}
			std::free(bounce_buffer);
		}
#endif
